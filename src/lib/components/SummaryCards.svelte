...
import { CheckCircleSolid, ClockSolid, ExclamationCircleSolid, PauseSolid } from "flowbite-svelte-icons";
...
function getStatusCounts(resources: K8sResource[]): Map<string, StatusCounts> {
  ...
  resources.forEach(resource => {
    ...
    if (!countsByKind.has(kind)) {
      countsByKind.set(kind, { ready: 0, notReady: 0, progressing: 0, suspended: 0 })
    }
...
$: totalFailingResources = Array.from(statusCounts.values()).reduce((sum, counts) => sum + counts.notReady, 0)
...
function handleStatusClick(status: string) {
  if (status === "Ready" || status === "NotReady" || status === "Progressing" || status === "Suspended") {
...
<div class="flex gap-4 overflow-x-auto pb-2">
  {#each displayedKinds as kind}
    {@const counts = statusCounts.get(kind) || { ready: 0, notReady: 0, progressing: 0, suspended: 0 }}
    <div 
      class="bg-white border border-slate-200 rounded-lg p-4 shadow-sm shrink-0 min-w-max hover:border-slate-300 hover:shadow transition-all cursor-pointer"
      onclick={() => handleKindClick(kind)}
      role="button"
      tabindex="0"
      onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleKindClick(kind); }}
    >
      ...
      <div class="flex items-center gap-3 text-sm">
        <div 
          ...
          onclick={(e) => { e.stopPropagation(); handleKindClick(kind); handleStatusClick("Ready"); }}
          role="button"
          tabindex="0"
          onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); handleKindClick(kind); handleStatusClick("Ready"); } }}
        >
        ...
        <div 
          ...
          onclick={(e) => { e.stopPropagation(); handleKindClick(kind); handleStatusClick("NotReady"); }}
          role="button"
          tabindex="0"
          onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); handleKindClick(kind); handleStatusClick("NotReady"); } }}
        >
        ...
        <div 
          ...
          onclick={(e) => { e.stopPropagation(); handleKindClick(kind); handleStatusClick("Progressing"); }}
          role="button"
          tabindex="0"
          onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); handleKindClick(kind); handleStatusClick("Progressing"); } }}
        >
          <ClockSolid size="sm" />
        ...
        <div 
          class="flex items-center gap-1.5 text-slate-400 hover:text-slate-500 cursor-pointer"
          onclick={(e) => { e.stopPropagation(); handleKindClick(kind); handleStatusClick("Suspended"); }}
          role="button"
          tabindex="0"
          onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); handleKindClick(kind); handleStatusClick("Suspended"); } }}
        >
          <PauseSolid size="sm" />
